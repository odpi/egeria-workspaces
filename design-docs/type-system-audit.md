...
| `static/egeria-shared-ui.js` | `EGERIA_EXPLORER_NAV` (~35 entries) + `resolveElementNav()` | Maps type names to the Egeria Explorer tab that displays them (falls back to walking `superTypeNames`), plus special-cases for `EngineAction`/Action-Center types/`ValidMetadataValue`. Canonical cross-app routing table as of BACKLOG.md NEXT-12 — used by egeria-operations/audit/insights/overview, and (duplicated by hand, see next row) the Portal's own search bar. **Not** loaded by `demo-portal.html` directly (top-level `React.createContext()` calls throw without React also loaded). | Same class of decision as `TYPE_TO_NAV` above — which Explorer tab owns a type is a UI choice, not a type-system fact. Two copies exist (quickstart/freshstart `static/`); confirmed identical at this table as of 2026-08-12, keep them that way. |
| `demo-portal.html` | `PORTAL_EXPLORER_NAV` (~35 entries) + `portalResolveNav()` | Hand-duplicated copy of `EGERIA_EXPLORER_NAV`/`resolveElementNav` above, for the Portal-level omni-search bar (NEXT-12). Exists because this plain-vanilla-JS landing page can't load the React-dependent shared file. | Same underlying decision as the row above, just physically duplicated for a real dependency reason (no React on this page), not by accident. **If you add a type to one table, add it to all four physical files (`egeria-shared-ui.js` × quickstart/freshstart, `demo-portal.html` × quickstart/freshstart) or Portal search and in-app cross-links will disagree on where a type goes.** |
...
- `EGERIA_EXPLORER_NAV`/`PORTAL_EXPLORER_NAV` disagree with each other, or with `TYPE_TO_NAV`, on where a type routes (check all four physical files, not just one)
