...
Four more `coco-workbooks` folders one level deeper than the six top-level ones also ship a manifest and need their own symlink into `dr-egeria-inbox` — `discover_batches()` only scans one level deep, so a nested folder is invisible unless it's symlinked in directly, same as any top-level one: `1. coco-data-hub/data-field-naming` ("Coco - Data Field Naming"), `1. coco-data-hub/extending-the-systems-inventory` ("Coco - Systems Inventory"), `1. coco-data-hub/strategic-digital-products` ("Coco - Strategic Digital Products"), and `4. keeping-safe/martyns-law` ("Coco - Martyn's Law").
