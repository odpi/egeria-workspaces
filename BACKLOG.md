...
2. **New "Local Dashboards" portal tile** (open) — a separate app/tile listing/selecting user-authored `DashboardSheet`s from the local JSON store (`~/.pyegeria/dashboard_sheets.json` via `dashboard_sheet_registry`). Splits into two honest phases: (a) metadata browser (name/heading/description/placements) — buildable now; (b) live chart rendering per placement — blocked on #3's execution engine. Not yet started; sequencing (build (a) now vs. wait for (b)) not yet decided.
