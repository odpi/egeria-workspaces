...
| NEXT-17 | **More comprehensive analytic metrics** (user engagement, definitions, and beyond the current 10) | open — **MEDIUM** | Raised 2026-07-30 alongside the Catalog Growth Trend demo spec review (egeria-python `overview_metrics.py`/`analytic_registry.py`/`analytic_demo_specs.py`). User feedback: "Good start on catalog growth trend — we need to think about more comprehensive metrics — including user engagement, definitions, etc." Today's 10 registered analytic functions (see `analytic_registry.py`) are all counts/breakdowns pulled from the Overview dashboard's existing metrics; growth_series is the only real time-series. Not scoped — needs a survey of what new metrics are worth adding (user engagement/activity, glossary/definition coverage, others) and which of the existing gaps flagged in the same review (per-function `as_of`/trend support, a real detail breakdown for `semantic_grounding`, a complete `context_readiness_funnel` definition) should land alongside new metrics vs. as their own items. |
