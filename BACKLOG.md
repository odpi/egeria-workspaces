...
| NEXT-14 | **Placement-level fixed parameters** (e.g. a scoped `search_string`, an explicit output format, a `Metadata Element Type` filter) | open — **MEDIUM** | Found 2026-07-29 authoring a real Local Dashboards placement: a user added `Output Format`/`Metadata Element Type` attributes to a `Link Report to Dashboard Sheet` command expecting them to control rendering — neither attribute exists on the `Report to Dashboard Sheet Link Base` bundle (own_attributes: `Dashboard Sheet Name`/`Placement Emphasis`/`Placement Span`/`Report Spec` only, see egeria-python `_output_dashboard_sheet_models.py`'s `Placement`), so they're silently dropped by the processor. **Confirmed live 2026-07-29** (not just inferred from the model): setting `### Output Format` to `TABLE` on a placement has zero effect — the tile still renders as `REPORT`, Local Dashboards' own fixed preference order, every time. Consequence: a Placement can only ever auto-run with an auto-filled blank `search_string` ("match everything, capped") — fine for narrow specs, but a broad one like the generic `Collections` spec then returns the *entire* dataset (observed 80–150KB from one placement). `local-dashboards.html` added a 20,000-char render cap as a stopgap (safe, but not a fix). Real fix needs: (1) add real attributes to the Link command bundle + `Placement` model (dr-egeria-command-sync skill territory — spans egeria-python + Tinderbox-authored compact command JSON), (2) `local_dashboards_handler.py`/`local-dashboards.html` read and use them instead of the fixed auto-fill/format-preference logic. Prerequisite-ish to NEXT-13 (an Advisor dashboard editor would want the same fields). **Note added 2026-07-31, status not changed — needs a human call on whether this counts as resolved:** `Output Format`/`Search String` (and now, as of the same date, `Report Parameters` for a required param outside the standard 22 — e.g. `collection_guid`) are no longer placement-level problems at all — the "Create Report" hard cutover (2026-07-29, predates this note) already lets a Report pin its own `Output Format`/`Search String`/etc. as defaults, and `Link Report to Dashboard Sheet` references that Report by name instead of a bare Report Spec. That's the *effect* this item asked for, just delivered at the Report-element level instead of the originally-proposed Link/Placement-bundle level — the two other examples this item names (`Metadata Element Type` filter, arbitrary non-search params) are also now coverable via `Create Report`'s existing 22-attribute vocabulary + `Report Parameters`. What's *not* covered: something that genuinely needs to vary **per placement of the same Report** (same Report, different scope on two different sheets) — that would still need Placement-level params, since today scope lives on the Report, not the Placement. Worth deciding whether that residual case still justifies "open — MEDIUM," or whether this closes as done with a new, narrower item opened only if that per-placement-override need actually comes up. |
...
| NEXT-18 | **Composite/derived analytic metrics** (e.g. a `sum_type_counts`-style wrapper) | open — **MEDIUM** | Raised 2026-07-30 re-assessing "is the Dashboard Sheet/Report/analytic-function track (OVERVIEW_REPORTING_MODEL.md §13) sufficient to rewrite the Overview dashboard." `overview_handler.py`'s `assetTotal = sum(r["count"] for r in counts_by_type(...))` has no analytic-function equivalent today — every registered function (`analytic_registry.py`) returns one function's raw result, none compose/derive a value across multiple calls or fields. A KPI like "total cataloged assets" (sum across several types) can't be expressed as a report spec yet. Not scoped — needs a design for how a composite analytic function declares/calls its dependencies (plain Python composition inside one new function is the obvious first answer; whether that generalizes further can wait for a second real example). |
| NEXT-19 | **Perspective lens for Dashboard Sheets / Placements** | open — **MEDIUM** | Raised 2026-07-30, same re-assessment as NEXT-18. Overview's `PERSP_KPIS`/`question_spec.perspectives` filters and reorders tiles per viewer role (`overview_containers.py`'s `view_for_perspective()`); `Placement`/`Dashboard Sheet` (`pyegeria/view/_output_dashboard_sheet_models.py`) has no equivalent — every Dashboard Sheet is a flat, unfiltered placement list today. Not scoped — likely needs a `perspectives` field on `Placement` or `Report` plus a Local Dashboards UI affordance to pick a perspective; relates to NEXT-13 (an Advisor dashboard editor would want the same concept). |
| NEXT-20 | **KPI-tile-band rendering mode + sparklines for Local Dashboards** | open — **MEDIUM** | Raised 2026-07-30, same re-assessment as NEXT-18. Local Dashboards (`local-dashboards.html`) renders every placement as a full card (heading + description + table/chart) — Overview's KPI band is compact icon+number+sparkline tiles in a row, a genuinely different, unbuilt rendering mode, not a CSS tweak. Not scoped — needs a new placement render mode (maybe driven by `Placement Emphasis`, which already distinguishes `"kpi"` vs `"panel"` but today only affects card sizing, not layout shape) plus a sparkline-capable analytic result shape (today's `SERIES` output format always renders a full standalone Vega-Lite chart panel, not an inline sparkline). |
| NEXT-21 | **Drill-down navigation for Local Dashboards placements** | open — **MEDIUM** | Raised 2026-07-30 in Egeria Explorer's Report Specs Run panel review ("we are likely going to need to extend this further to support master-detail drill downs"), reconfirmed in the NEXT-10-successor re-assessment (OVERVIEW_REPORTING_MODEL.md §5's "known gap" — Vega bars lost the old `hbars()` per-bar click-to-drill affordance, and it still applies to the newer Dashboard Sheet track too). Every Overview tile has a `detail_spec` click-through; Local Dashboards' `Report`/`Placement` model has no equivalent concept, and it's unclear what a drill target even means for an analytic-function-backed report (there's no natural "detail" for an aggregated count the way there is for a find-path element list). Not scoped — needs its own design pass, not a bolt-on. |
| NEXT-22 | **Caching for Local Dashboards placement execution** | open — **MEDIUM** | Raised 2026-07-30, same re-assessment as NEXT-18. `overview_handler.py` caches aggressively (60–900 s TTLs, `growth_series` especially — N asOfTime snapshots is genuinely expensive) precisely because some queries are costly; every Local Dashboards placement (`local_dashboards_handler.py`) re-queries live on every page load, no caching at all. Fine for today's handful of demo placements, not fine for an Overview-scale dashboard (11+ tiles) refreshed on every visit. Not scoped — could likely reuse `overview_handler.py`'s own cache-key/TTL pattern rather than inventing a new one. |
