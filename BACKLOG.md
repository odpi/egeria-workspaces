...
| TC-10 | Governance zones: display and filter — `ZoneMembership` is a classification (not a separate API mechanism); zones appear automatically in the Classifications section once TC-11 lands; add zone membership as a sidebar filter option (type: `ZoneMembership`, property: `zoneMembershipList`) alongside other classification filters; zones can be used for access control (visibility restricted) or pure grouping; elements can belong to multiple zones; zones can be nested | open | Depends on TC-11; zone security may mean some elements are invisible to certain users (handled by TC-3's 403 detection); no separate find_* parameter needed |
| TC-11 | Classification ubiquity audit and fix — classifications must be populated in API responses for ALL element types (assets, endpoints, software capabilities, glossary terms, folders, glossaries, tech types); current issue: lightweight list/detail endpoints may not populate `elementHeader.classifications`; audit each endpoint's response structure; if depth parameter or separate call needed to retrieve classifications, add it; `_extract_classifications` is already wired to all serializers but the underlying data may be absent | open | **Blocks TC-10 and TC-12**; also blocks accurate zone and security classification display; investigate by logging raw API responses for known classified elements in the Coco demo |
| TC-12 | Classification-based sidebar filtering — add filter chips or a filter panel to the sidebar in all catalog sections; user selects one or more classification type+value combinations to narrow the result list; examples: filter by `Criticality.criticalityLevel=CRITICAL_IMPACT`, `ZoneMembership` (any zone), `SubjectArea`; filtering can be client-side (filter already-loaded items) for small result sets; design: show classification types present in the current result set as collapsible filter groups | open | Depends on TC-11 (classifications must be in item data); consider showing classification badges on sidebar list items as a quick overview |
