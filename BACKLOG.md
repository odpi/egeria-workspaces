...
**Known gap found while verifying the above, not yet fixed:** PrimeWord/ClassWord/Modifier, once applied, don't show up in `GlossaryTermDetail`'s existing "Classifications" section (which already correctly displays Confidentiality/Criticality/Impact/Confidence — that display code predates this session). Traced to the retrieval side, not display: neither `GlossaryManager.get_term_by_guid` nor `ClassificationExplorer.get_element_by_guid` return these classifications in `elementHeader` at any `graphQueryDepth` (0/1/2 all tried live), even though `get_elements_by_classification('PrimeWord')` finds the element fine — i.e. it's genuinely attached, just not surfaced by element-retrieval. Looks like an Egeria/pyegeria serialization gap specific to this classification family, not something fixable in this app's display/serialization code (see `pyegeria_issues.md` memory). Revisit if/when an upstream fix lands or a different retrieval call is found that does surface it.
...
| PY-23 | `Create Information Supply Chain`'s `Purposes` attribute validates/processes with `SUCCESS` but is never persisted to the element — confirmed live creating 17 ISCs, none retained their `Purposes` value | **open** — found 2026-08-18 building `gen_governance_metrics.py`. See PYEGERIA_ISSUES.md ISSUE-62 | Workaround: use the standard `Description` attribute instead — reliably persisted |
