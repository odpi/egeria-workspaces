...
| NEXT-16 | **Rename "Usage Context Counts" analytic function/demo spec** | open — **MEDIUM** | Raised 2026-07-30 reviewing the Analytic Function Demo report specs (egeria-python `overview_metrics.usage_context_counts`, `analytic_registry.py`, `analytic_demo_specs.py` — InformationSupplyChain + SolutionBlueprint counts). User feedback: "Not sure this is the best name... we need to think of a better one." Not scoped — needs a naming discussion (candidates should describe what puts assets in a *usage context*, e.g. something in the space of "Usage Context Coverage" / "Supply Chain & Blueprint Counts" / etc.) before touching the registry entry, demo spec name, and heading/description together. |
