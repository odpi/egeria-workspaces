...
| NEXT-8 | **Overview dashboard: define "AI-ready" per AI purpose** | **design done 2026-08-01, ready to build** | Resolved: no universal AI-readiness confirmed (matches Gartner). Replacement is a **Data Lens Conformance report spec** (parameterized per project/purpose), not an Overview KPI — lives in Report Spec browser / a scoped Local Dashboards placement. Data Scope/Grain/Lens now fully defined, grounded in the PDR Context Intelligence blog series. See `OVERVIEW_CONTEXT_INTELLIGENCE.md` §2.5, Plan Phase C item 9. Detail: OVERVIEW_NEXT_STEPS.md R-2 |
...
| NEXT-23 | **Overview: per-perspective section CONTENT variants** (e.g. "Usage Context — Privacy Officer" vs. "— Data Owner") | open — **MEDIUM** | Raised 2026-08-01, following the Perspective × Topic two-axis navigation build (both axes now control section visibility *and* the KPI band — see OVERVIEW_NEXT_STEPS.md R-4). Deliberate design call: a shown section's content should vary by authoring distinct per-perspective/topic section *variants*, not by runtime-filtering one shared section — most section content is hardcoded HTML, not in the tile registry, so filtering-in-place would need a large registry migration or a second parallel tagging system (drift risk); variants are additive instead. Not scoped — needs a decision on the lookup key shape (per-Perspective, per-Topic, or full cross product) and a first concrete example before generalizing. Detail: OVERVIEW_NEXT_STEPS.md R-4 |
