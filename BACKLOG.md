...
| PY-10 | **Asset detail by-guid rejects `asOfTime`.** `AssetCatalog.get_asset_graph_by_guid` (body `ResultsRequestBody`+`asOfTime`) → `SERVER_ERROR_500`; `AssetMaker.get_asset_by_guid` (`/assets/{guid}/retrieve`, body `GetRequestBody`+`asOfTime`) → `404 InvalidParameterException` — both at a recent time the asset clearly existed (baseline with no body works). So point-in-time works on `find_*` searches but **not** the asset by-guid retrieve/graph endpoints. Blocks LE-3 time-travel on the Tech Catalog **asset detail pane**. | open (Dan) | Surfaced 2026-06-21. Likely an Egeria asset-catalog/asset-manager OMVS limitation or wrong request-body class for as-of-time on these endpoints. |
