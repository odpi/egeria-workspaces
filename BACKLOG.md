...
| NEXT-24 | **Govern the dashboard's own metrics: audit + Glossary + Collections + info bubble** | **design done 2026-08-01, ready to build Phase A** | Triggered by a live finding while drilling into Semantic Grounding: 87.7% of its `SemanticAssignment` relationships connect to `GovernanceActionProcess` elements, not data assets — zero connect to an `Asset` directly — so the tile's label doesn't match what it actually counts. Design: generate (not hand-author) a `GlossaryTerm` per metric from `overview_specs.py`'s own tile registry (mirrors the existing `gen_perspectives.py` pattern exactly — Python stays the single source of truth), each Term carrying real `summary`/`description`/`usage` fields (`usage` is where caveats like the grounding finding live structurally). Terms are grouped under a new **RootCollection "Egeria Dashboard"** with sub-collections (by app, by provenance for v1; Topic/Perspective sub-collections as cheap later additions, since Egeria collection membership is many-to-many). Feeds a dashboard info-bubble UI. Full design + phased plan: `OVERVIEW_METRIC_GOVERNANCE.md`. |
