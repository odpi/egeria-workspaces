...
Adapted from egeria-python/tests/scenario-tests/run_dr_tests.py: that
version resolves TEST_FILES relative to the configured Dr.Egeria Inbox path,
which mixes test fixtures into a working scratch folder. This version uses
absolute paths (process_md_file_v2's input_file accepts an absolute path or
one relative to EGERIA_INBOX_PATH) so the markdown fixtures and this runner
can live together under egeria-python/tests/, self-contained and reviewable
...
# Add new test files here as commands are added/changed. Use just the
# filename -- they're resolved relative to THIS_DIR below.
TEST_FILES = [
    # "dr_test_<family_or_topic>.md",
]
...
def main():
    if not TEST_FILES:
        print("TEST_FILES is empty -- add dr_test_*.md filenames before running.")
        sys.exit(1)
