...
def display_catalog(url:str):
    ...
    def generate_table() -> Table:
        ...
        for catalog in catalogs:
            ...
            cat_info = uc_client.catalogs.retrieve(cat_name)
            schemas = None
            schema = None
            ...
            for schema in schemas:
                ...
                if len(uc_tables) > 0:
                    for tab in uc_tables:
                        ...
                        columns = tab.columns
                        col_m = " "
                        col_tab = Table(title=f"{tab_name}",expand=True)
                        ...
                        # col_tabs = f"{col_tabs}\n{col_tab}"
                        # tables_m = f"{tables_m}\n{table_m}"
                        table.add_row(cat_name, cat_comment, schema_name, schema_comment, Text(table_m),
                                      col_tab)
                if (len(volumes) == 0) and (len(functions) == 0) and (len(uc_tables) == 0):
                    table.add_row(cat_name, cat_comment, schema_name, schema_comment," "," ")
            # else:
            #     table.add_row(cat_name, cat_comment, schema_name, schema_comment, f"{len(volumes)}, {len(functions)}, {len(uc_tables)} ", " ")
        return table
