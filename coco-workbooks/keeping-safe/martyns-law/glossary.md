This file creates a glossary for Martyn's Law and glossary terms for key definitions introduced by the regulation.  The terms include:

- [Appropriate](#appropriate)
- [At the same time](#at-the-same-time)
- [Communication](#communication)
- [Compliance notice](#compliance-notice)
- [Co-operation](#co-operation)
- [Co-ordination](#co-ordination)
- [Enhanced duty/enhanced tier](#enhanced-tier)
- [Evacuation](#evacuation)
- [From time to time](#from-time-to-time)
- [Immediate vicinity](#immediate-vicinity)
- [Invacuation](#invacuation)
- [Lockdown](#lockdown)
- [The National Counter-Terrorism Security Office (NaCTSO)](#the-national-counter-terrorism-security-office)
- [The National Protective Security Authority (NPSA)](#the-national-protective-security-authority)
- [Penalty notice](#penalty-notice)
- [Principal use](#principal-use)
- [Public protection measures](#public-protection-measures)
- [Public protection procedures](#public-protection-procedures)
- [Qualifying event](#qualifying-event)
- [Qualifying premises](#qualifying-premises)
- [Reasonably practicable](#reasonably-practicable)
- [Responsible person or responsible persons](#responsible-person)
- [Restriction notice](#restriction-notice)
- [Schedule 1 use](#schedule-1-use)
- [Scope](#scope)
- [Security Industry Authority (SIA)](#security-industry-authority)
- [Senior individual](#senior-individual)
- [Staff](#staff)
- [Standard duty or standard tier](#standard-tier)
- [Vulnerability](#vulnerability)
- [Wholly or mainly](#wholly-or-mainly)


____
## Create Glossary  

### Display Name  

Glossary for the Terrorism (Protection of Premises) Act 2025 (UK) 
### Language  

English
### Usage  
 
This glossary should be used to clarify Coco Pharmaceutical's responsibilities under the UK Terrorism (Protection of Premises) Act.
### Category  

In-person Meetings and Events  

### Description  

This glossary covers the terminology used in the UK Terrorism (Protection of Premises) Act, also known as Martyn's Law.  This act contains very precise definitions for premises and events based on their number of attendees (including staff).  This glossary provides clarification on these terms in the context of Coco Pharmaceuticals.
### Qualified Name  

Glossary::MartynsLawUK
  
### Content Status  

ACTIVE
### Purpose  

To support Coco Pharmaceutical's response to the UK Terrorism (Protection of Premises) Act.
  
### Search Keywords  

- UK Regulations
- Physical Events
- Counter-Terrorism

### Version Identifier  
 
1.0

### Authors  

- Ivor Padlock
____


# Appropriate

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Appropriate

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Appropriate means suitable. Those responsible for qualifying premises and events must ensure that procedures and/or measures are appropriate, taking account of the specific context of the premises or event.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Appropriate

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Appropriate

### Pages

See paras. 7.21 and 8.24.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____
## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Appropriate

### Pages

Section 5, section 6 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# At the same time

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

At the same time

### Glossary Name

- Glossary::MartynsLawUK

### Summary

This is a common term and is understood to mean together or simultaneously. In the Act, the term refers to individuals being present on the premises together at any one time.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::AtTheSameTime

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::AtTheSameTime
### Pages

See paras. 4.31 to 4.34.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

# Communication

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Communication

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Communication is one of the four types of public protection procedure set out in the Act. Communication involves providing information to people on the premises or at the event, alerting people to danger and providing instructions, where it is safe to do so.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Communication

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____
## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Communication
### Pages

See paras. 7.46 to 7.49.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Communication

### Pages

Section 5 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Compliance notice

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Compliance notice

### Glossary Name

- Glossary::MartynsLawUK

### Summary

A statutory notice issued by the SIA requiring the recipient to comply with a requirement of the Act within a specified period.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::ComplianceNotice

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____
## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::ComplianceNotice

### Pages

See para. 9.10.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::ComplianceNotice

### Pages

Section 13 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Co-operation

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Co-operation

### Glossary Name

- Glossary::MartynsLawUK

### Summary

In some cases, an individual or organisation can have some level of control of premises in the enhanced tier or a qualifying event without being the responsible person. In these circumstances, they must co-operate with the responsible person, so far as is reasonably practicable, to help meet the Act’s requirements.

Additionally, where premises form part of larger qualifying premises, they may need to co‑operate with the responsible person for the larger premises for the purposes of that responsible person complying with the requirements of the Act.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Co-operation

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____
## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Co-operation

### Pages

See paras. 6.21 to 6.22.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Co-operation

### Pages

Section 8 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Co-ordination

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Co-ordination

### Glossary Name

- Glossary::MartynsLawUK

### Summary

In some cases, there may be multiple responsible persons for qualifying premises or events. They must co-ordinate with each other, so far as is reasonably practicable, to meet the Act’s requirements.

Additionally, where qualifying premises form part of other qualifying premises, the responsible persons must co-ordinate with each other, so far as is reasonably practicable, to meet the Act’s requirements.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Co-ordination

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Co-ordination

### Pages

See paras. 6.16 to 6.20.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Co-ordination

### Pages

Section 8 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Enhanced tier
____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Enhanced tier

### Aliases

- Enhanced duty
- enhanced tier premises
- Enhanced duty premises

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Larger premises or events hosting 800 or more individuals.

### Usage

This term is used to classify premises to determine which set of requirements need to be met.

### Category

In-person Meetings and Events

### Description

The Act establishes a tiered approach for premises – the standard duty and the enhanced duty (also known as the standard and enhanced tiers). Enhanced tier premises are qualifying premises that can reasonably expect 800 or more individuals to be present at any one time, from time to time, for the purpose of a Schedule 1 use.

In addition to the requirements for the standard tier, these larger premises and events will have further requirements placed upon them. This includes having in place, so far as is reasonably practicable, appropriate public protection measures to reduce their vulnerability to acts of terrorism and the risk of physical harm if an act of terrorism was to occur. Appropriate measures for enhanced duty premises and events could include bag search policies, CCTV, or other monitoring and vehicle checks.

See chapter 4 for the criteria to be in scope of the Act as qualifying premises (and paras. 4.35 to 4.39 for excluded premises and those subject to different thresholds). See chapter 5 for the criteria to be in scope of the Act as a qualifying event and chapter 8 on enhanced tier requirements.

### Qualified Name

GlossaryTerm::MartynsLawUK::EnhancedTier

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::EnhancedTier

### Pages

See chapter 4.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::EnhancedTier

### Pages

Section 2(2), section 2(3) of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::EnhancedTier

### External Reference

ExternalReference::MartynsLawUK::EnhancedDutyRequirements


### Label

Factsheet

### Description

A factsheet summarising information about the enhanced duty requirements.

____

# Evacuation

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Evacuation

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Evacuation is one of the four types of public protection procedure set out in the Act. Evacuation involves getting people away from danger by moving them out of the premises or event (or part of the premises or event).

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Evacuation

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Evacuation

### Pages

See paras. 7.33 to 7.38.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Evacuation

### Pages

Section 5 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# From time to time

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

From time to time


### Glossary Name

- Glossary::MartynsLawUK

### Summary

This means occasionally. In the Act, the phrase ‘from time to time’ is used within the definition of qualifying premises and it refers to how often 200 or more individuals (including staff) can reasonably be expected to be present on the premises at the same time.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::FromTimeToTime

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::FromTimeToTime
### Pages

See paras. 4.31 to 4.34.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::FromTimeToTime

### Pages

Section 2 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____


# Immediate vicinity

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Immediate vicinity

### Glossary Name

- Glossary::MartynsLawUK

### Summary

This is an area close to the premises. There is no fixed distance associated with the term.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::ImmediateVicinity

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::ImmediateVicinity
### Pages

See paras. 7.12 to 7.14.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::ImmediateVicinity

### Pages

Section 5, section 6 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Invacuation

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Invacuation

### Glossary Name

- Glossary::MartynsLawUK

### Summary

This is one of the four types of public protection procedure set out in the Act. Invacuation involves moving people away from danger to a place within the premises or event where there is less risk of physical harm being caused to them. This can include bringing people into the premises or event from outside or moving people from one part of the premises or event to another, where it is safe to do so.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Invacuation

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Invacuation

### Pages

See paras. 7.39 to 7.40.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Invacuation

### Pages

Section 5 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Lockdown

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Lockdown

### Glossary Name

- Glossary::MartynsLawUK

### Summary

This is one of the four types of public protection procedure set out in the Act. Lockdown is the process of securing the premises or event, to prevent individuals entering or leaving.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Lockdown

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Lockdown

### Pages

See paras. 7.41 to 7.45.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Lockdown

### Pages

Section 5 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# The National Counter-Terrorism Security Office

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

The National Counter-Terrorism Security Office

### Abbreviation

NaCTSO

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Police-hosted unit supporting the Protect and Prepare strands of the UK government’s counter-terrorism strategy. NaCTSO manages the ProtectUK website, a source of relevant guidance. [[footnote 14]](https://www.gov.uk/government/publications/the-terrorism-protection-of-premises-act-2025/terrorism-protection-of-premises-act-2025-statutory-guidance#fn:14)

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::NationalCounterTerrorismSecurityOffice

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::NationalCounterTerrorismSecurityOffice

### External Reference

ExternalReference::MartynsLawUK::ProtectUKMartynsLaw::HomePage

### Label

Factsheet

### Description

Home page of Martyn's Law on the ProtectUK website

____

# The National Protective Security Authority

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

The National Protective Security Authority

### Abbreviation

NPSA

### Glossary Name

- Glossary::MartynsLawUK

### Summary

The UK government’s national technical authority for physical and personnel protective security. The NPSA website is a source of protective security guidance. [[footnote 15]](https://www.gov.uk/government/publications/the-terrorism-protection-of-premises-act-2025/terrorism-protection-of-premises-act-2025-statutory-guidance#fn:15)


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::NationalProtectiveSecurityAuthority

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::NationalProtectiveSecurityAuthority

### External Reference

ExternalReference::NationalProtectiveSecurityAuthority::HomePage

### Label

Factsheet

### Description

Home page of Martyn's Law on the ProtectUK website

____

# Penalty notice

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Penalty notice

### Glossary Name

- Glossary::MartynsLawUK

### Summary

A statutory notice requiring a specified amount to be paid to the SIA. This may include daily penalties for continued non-compliance.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::PenaltyNotice

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::PenaltyNotice

### Pages

See paras. 9.12 to 9.16.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::PenaltyNotice

### Pages

Section 17, section 19, section 21 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____


# Principal use

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Principal use

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Where premises have multiple Schedule 1 uses (set out in Schedule 1 to the Act), the relevant Schedule 1 use is whichever of those uses is the principal use of the premises. The individual or organisation with control of the premises for the purposes of its principal Schedule 1 use is the responsible person.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::PrincipalUse

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::PrincipalUse
### Pages

See paras. 4.17 to 4.19 and paras. 6.3 to 6.5.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::PrincipalUse

### Pages

Section 4 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Public protection measures

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Public protection measures

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Public protection measures are measures relating to monitoring, movement, physical safety and security, and security of information. These measures seek to reduce the risk of physical harm to people and reduce the vulnerability of the premises or event to terrorist attacks. Public protection measures are relevant to enhanced tier premises and qualifying events.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::PublicProtectionMeasures

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::PublicProtectionMeasures
### Pages

See paras. 8.7 to 8.55.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::PublicProtectionMeasures

### Pages

Section 6 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Public protection procedures

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Public protection procedures

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Public protection procedures are sets of actions that staff can take if they suspect an act of terrorism is occurring, or is about to occur, at the premises, at the event or in the immediate vicinity. The procedures are evacuation, invacuation, lockdown and communication. Public protection procedures seek to reduce the risk of physical harm to people. Public protection procedures are relevant to standard tier premises, enhanced tier premises and qualifying events.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::PublicProtectionProcedures

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::PublicProtectionProcedures

### Pages

See paras. 7.4 to 7.62.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::PublicProtectionProcedures

### Pages

Section 5 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Qualifying event

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Qualifying event

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Events meeting the criteria set out in section 3 of the Act and subject to enhanced duty requirements.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::QualifyingEvent

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____
## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingEvent
### Pages

See chapter 5.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingEvent

### Pages

Section 3 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingEvent

### External Reference

RelatedMedia::MartynsLawUK::Scope::Events::Infographic

### Label

Infographic

### Description

Flowchart of the events covered by the Act.

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingEvent

### External Reference

ExternalReference::MartynsLawUK::Scope::Events

### Label

Factsheet

### Description

Summary of the events covered by the Act.

____

# Qualifying premises

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Qualifying premises

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Premises meeting the criteria set out in section 2 of the Act. Depending on the number of individuals that can reasonably be expected to be on the premises, qualifying premises may be subject to standard or enhanced duty requirements.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::QualifyingPremises

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingPremises

### Pages

See chapter 4.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingEvent

### Pages

Section 2 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingPremises

### External Reference

RelatedMedia::MartynsLawUK::Scope::Premises::Infographic

### Label

Infographic

### Description

Flowchart of the premises covered by the Act.

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::QualifyingPremises

### External Reference

ExternalReference::MartynsLawUK::Scope::Premises

### Label

Factsheet

### Description

Summary of the premises covered by the Act.

____

# Reasonably practicable

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Reasonably practicable

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Reasonably practicable means proportionate. In considering what is reasonably practicable, the responsible person should weigh what can be done to achieve the objectives of procedures and/or measures, balanced against the cost, time and difficulty of implementation.  The responsible person must consider what is reasonably practicable when ensuring that appropriate public protection procedures and/or measures are in place.

### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::ReasonablyPracticable

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::ReasonablyPracticable

### Pages

See paras. 7.20 to 7.28, 8.18 to 8.20, 8.25.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

# Responsible person

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Responsible person or responsible persons

### Glossary Name

- Glossary::MartynsLawUK

### Summary

For qualifying premises, this is the individual, organisation or company with control of the qualifying premises for the purpose of its Schedule 1 use. For qualifying events, this is the individual, organisation or company with control of the premises at which the qualifying event is taking place, for the purposes of the event. The responsible person must ensure that the requirements of the Act are met.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::ResponsiblePerson

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::ResponsiblePerson
### Pages

See chapter 6.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::ResponsiblePerson

### Pages

Section 4 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::ResponsiblePerson

### External Reference

ExternalReference::MartynsLawUK::ResponsiblePerson

### Label

Infographic

### Description

Flowchart of the events covered by the Act.

____


# Restriction notice

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Restriction notice

### Glossary Name

- Glossary::MartynsLawUK

### Summary

A statutory notice requiring the recipient to comply with specified restrictions relating to the use of the premises or the continuation of the event. This is issued by the SIA.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::RestrictionNotice

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::RestrictionNotice

### Pages

See para. 9.11.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::RestrictionNotice

### Pages

Section 14 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Schedule 1 use

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Schedule 1 use

### Glossary Name

- Glossary::MartynsLawUK

### Summary

A Schedule 1 use is one of the specific types of activities listed in Schedule 1 to the Act. Premises that are wholly or mainly used for any of these activities may fall within the scope of the legislation, if they meet the other criteria under section 2 of the Act, and therefore become qualifying premises.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Schedule1Use

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::Schedule1Use
### Pages

See para. 4.17 and figure 3.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Schedule1Use

### Pages

Schedule 1 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Scope

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Scope

### Glossary Name

- Glossary::MartynsLawUK

### Summary

This means the boundaries or coverage of the Act (in essence, what is covered under the Act and what is not covered).


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Scope

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

# Security Industry Authority

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Security Industry Authority

### Abbreviation

SIA

### Glossary Name

- Glossary::MartynsLawUK

### Summary

The regulator for the Terrorism (Protection of Premises) Act 2025.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::SecurityIndustryAuthority

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock

____

# Senior individual

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Senior individual

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Where the responsible person for enhanced tier premises or a qualifying event is an organisation or company, a senior individual must be designated to ensure compliance with the Act. The senior individual is someone who is involved in the management or control of the responsible person.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::SeniorIndividual

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::SeniorIndividual
### Pages

See paras. 6.8 to 6.12 and paras. 8.69 to 8.74.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::SeniorIndividual

### Pages

Section 10 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Staff

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Staff

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Throughout this guidance, staff means all those working at qualifying premises or events, including employees, contractors, volunteers and others. Staff may be full time, part time, shift workers or follow other working patterns.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Staff

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____


# Standard tier

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Standard tier

### Aliases

- Standard duty
- Standard tier premises
- Standard duty premises

### Glossary Name

- Glossary::MartynsLawUK

### Summary

The Act establishes a tiered approach for premises – the standard duty and the enhanced duty (also known as the standard and enhanced tiers). Standard tier premises are qualifying premises that can reasonably expect 200 to 799 individuals to be present at the same time, from time to time, for the purpose of a Schedule 1 use.

### Usage

This term is used to classify premises to determine which set of requirements need to be met.

### Category

In-person Meetings and Events

### Description

Smaller premises where 200 to 799 individuals may be present will be in the ‘standard tier’. The requirements in this tier are centred on simple, low-cost activities designed to ensure those working at premises or events are better able to reduce harm, and save lives, in the event of an attack.

These activities could be as simple as locking doors, closing shutters, and identifying safe routes to cover. There is no requirement to put in place physical measures in the standard tier.

### Qualified Name

GlossaryTerm::MartynsLawUK::StandardTier

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock
____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name

GlossaryTerm::MartynsLawUK::StandardTier

### Pages

See chapter 4 for the criteria to be in scope of the Act as qualifying premises (and paras. 4.35 to 4.39 for excluded premises and those subject to different thresholds) and chapter 7 on standard tier requirements.

### Reference Id

Principal Source

### Description

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::StandardTier

### Pages

Section 6 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

## Link External Reference
> Link an external reference to a referenceable.

### Element Name

GlossaryTerm::MartynsLawUK::StandardTier

### External Reference

ExternalReference::MartynsLawUK::StandardDutyRequirements


### Label

Factsheet

### Description

A factsheet summarising information about the standard duty requirements.

____

# Vulnerability

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Vulnerability

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Weaknesses that may be exploited by an attacker to achieve an impact, such as harm to people or an organisation. The presence of a threat is required to exploit a vulnerability.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::Vulnerability

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::Vulnerability

### Pages

Section 1(1)(b) of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____

# Wholly or mainly

____

## Create Glossary Term
> Creates or updates a glossary term — a concept, phrase, or word defined within a glossary.

### Display Name

Wholly or mainly

### Glossary Name

- Glossary::MartynsLawUK

### Summary

Wholly or mainly means entirely or mostly. This is one of the criteria for qualifying premises – that the premises are wholly or mainly used for a use set out in Schedule 1 to the Act.


### Usage

This term is used to understand the requirements of Martyn's Law.

### Category

In-person Meetings and Events

### Description


### Qualified Name

GlossaryTerm::MartynsLawUK::WhollyOrMainly

### Content Status

ACTIVE

### Version Identifier

1.0

### Authors

- Ivor Padlock


____

## Link Cited Document  
> Link a cited document to a referenceable via the CitedDocumentLink relationship.  

### Cited Document  

CitedDocument::MartynsLawUK::StatutoryGuidance

### Element Name  

GlossaryTerm::MartynsLawUK::WhollyOrMainly
### Pages  

See paras. 4.17 to 4.21.

### Reference Id  

Principal Source

### Description  

Text extracted from Home Office's Statutory Guidance Glossary (Chapter 3).

____

## Link Cited Document
> Link a cited document to a referenceable via the CitedDocumentLink relationship.

### Cited Document

CitedDocument::MartynsLawUK::TheAct

### Element Name

GlossaryTerm::MartynsLawUK::WhollyOrMainly

### Pages

Section 2 of the Act

### Reference Id

The Act

### Description

Link to the original Act.

____
