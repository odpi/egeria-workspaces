...
> HTTPS is self-signed by default (see [Portal and Authentication](../README.md#portal-and-authentication)) — `curl` needs `-k`/`--insecure` against these example URLs unless `CERT_DIR` points to a real certificate.
...
```
GET https://localhost:7843/api/types
GET https://localhost:7843/api/types?area=4
```
...
```
http://localhost:7800/egeria-audit
```
...
```
http://localhost:7800/egeria-operations
```
