...
JupyterLab session lock — manages exclusive access to the shared JupyterLab
instance so multiple demo users don't collide.
...
require the admin role.  Outside DEMO_MODE all endpoints are open.
"""
...
router = APIRouter(prefix="/api/jupyter", tags=["jupyter-lock"])

# ── Configuration ──────────────────────────────────────────────────────────────

LOCK_ENABLED         = os.environ.get("JUPYTER_LOCK_ENABLED", "true").lower() not in ("false", "0", "no")

_SESSION_MINUTES     = int(os.environ.get("JUPYTER_SESSION_MINUTES",     "60"))
_IDLE_SOFT_MINUTES   = int(os.environ.get("JUPYTER_IDLE_SOFT_MINUTES",   "15"))
_IDLE_HARD_MINUTES   = int(os.environ.get("JUPYTER_IDLE_HARD_MINUTES",   "30"))
_BUFFER_MINUTES      = int(os.environ.get("JUPYTER_BUFFER_MINUTES",      "10"))
_EVICT_GRACE_SECONDS = int(os.environ.get("JUPYTER_EVICT_GRACE_SECONDS", "300"))
_CLEANUP_INTERVAL    = 60   # background task interval in seconds

_LOCK_FILE = Path(os.environ.get(
    "JUPYTER_LOCK_STATE_FILE",
    "/app/demo-data/jupyter_lock.json",
))
...
def _save_state() -> None:
    try:
        ...
        logger.warning(f"jupyter lock: could not persist state: {exc}")
...
def _load_state() -> None:
    try:
        if _LOCK_FILE.exists():
            ...
            _state.update(data)
            logger.info(f"jupyter lock: restored state '{_state['state']}' from disk")
    except Exception as exc:
        logger.warning(f"jupyter lock: could not load saved state: {exc}")
...
def _startup_cleanup() -> None:
    ...
    if _state.get("state") not in ("IN_USE", "ADMIN_IN_USE", "STUCK"):
        return
    if not DEMO_MODE:
        logger.info("jupyter lock: local mode — clearing lock left from previous run")
        _do_release("startup_cleanup")
        return
    now = datetime.utcnow()
    exp = _parse_iso(_state.get("expires_at"))
    if exp and now > exp:
        logger.info("jupyter lock: releasing expired session from previous run")
        _do_release("startup_cleanup")
        return
    ka = _parse_iso(_state.get("last_keepalive"))
    if ka and (now - ka).total_seconds() > _IDLE_HARD_MINUTES * 2 * 60:
        logger.info("jupyter lock: releasing stale session from previous run (keepalive dead)")
        _do_release("startup_cleanup")
...
async def start_scheduler() -> None:
    ...
    logger.info("jupyter lock scheduler started")
...
async def _cleanup_loop() -> None:
    while True:
        ...
        try:
            ...
            logger.error(f"jupyter lock cleanup error: {exc}")
...
async def _run_cleanup() -> None:
    async with _mu:
        ...
        if state in ("IN_USE", "ADMIN_IN_USE"):
            exp = _parse_iso(_state.get("expires_at"))
            if exp and now > exp:
                logger.info("jupyter lock: session expired — releasing")
                _do_release("expired")
                return
            if _state.get("evicting"):
                deadline = _parse_iso(_state.get("evict_deadline"))
                if deadline and now > deadline:
                    logger.info("jupyter lock: eviction grace period over — releasing")
                    _do_release("evicted")
                    return
            ka = _parse_iso(_state.get("last_keepalive"))
            if ka:
                ...
                if idle_seconds > _IDLE_HARD_MINUTES * 60:
                    logger.info(f"jupyter lock: idle {idle_seconds:.0f}s — marking STUCK")
                    _state["state"] = "STUCK"
                    _save_state()
            ...
            if ka:
                ...
                if idle_seconds > _IDLE_HARD_MINUTES * 2 * 60:
                    logger.info("jupyter lock: stuck session auto-released")
...
def _require_lock_enabled():
    if not LOCK_ENABLED:
        raise HTTPException(status_code=503, detail="Jupyter lock is disabled on this instance (JUPYTER_LOCK_ENABLED=false)")
...
@router.get("/status")
async def jupyter_status(request: Request):
    """Public — current lock state and time remaining."""
    if not LOCK_ENABLED:
...
async def acquire(request: Request, body: AcquireRequest, _: None = Depends(_require_lock_enabled)):
    """Acquire the Jupyter lock. Returns a session token for keepalive/release."""
    ...
    async with _mu:
        ...
        if state not in ("FREE",):
            ...
            return {
                "acquired": False,
                "reason": f"Jupyter is {state.lower().replace('_', ' ')}",
        ...
        _save_state()
        logger.info(f"jupyter lock acquired by '{display}'")
        return {
            "acquired":        True,
            "token":           token,
...
async def release(request: Request, body: ReleaseRequest, _: None = Depends(_require_lock_enabled)):
    ...
    async with _mu:
        ...
        logger.info("jupyter lock released by holder")
...
async def create_reservation(request: Request, body: ReservationRequest, _: None = Depends(_require_lock_enabled)):
    ...
    logger.info(f"jupyter reservation created: {body.label} ({body.starts_at} → {body.ends_at})")
    return entry
...
async def evict(request: Request, body: EvictRequest, _: None = Depends(_require_lock_enabled)):
    ...
    async with _mu:
        ...
        _save_state()
        logger.info(f"jupyter lock eviction started — grace {grace}s, deadline {deadline}")
        return {
            "evicting":       True,
            "evict_deadline": deadline,
            "grace_seconds":  grace,
            "message":        body.message or f"An admin needs Jupyter in {grace // 60} min. Please save your work.",
        }
...
async def force_unlock(request: Request, _: None = Depends(_require_lock_enabled)):
    ...
    async with _mu:
        ...
        logger.info("jupyter lock force-unlocked by admin")
