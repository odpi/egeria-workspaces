...
| Remote (any mode) | `https://<HOST_FQDN>:3001` | nginx inside the container serves HTTPS on port 3001 with a self-signed cert — accept the browser warning once |
| Demo mode (remote, real cert) | `https://<HOST_FQDN>:3001` | Still self-signed for now; the real-cert Apache proxy is a future improvement |

> **Accepting the self-signed cert**: navigate to `https://cray:3001` in your browser, click through the "your connection is not private" warning (Advanced → Proceed). The cert is auto-generated by the linuxserver base image and covers port 3001 only.

### Vault access

The container mounts `coco-workbooks/` at `/config/vaults/coco-workbooks`. On first launch, open that vault inside the Obsidian UI (File → Open vault → `/config/vaults/coco-workbooks`). Settings are persisted in `runtime-volumes/obsidian-config/`.
