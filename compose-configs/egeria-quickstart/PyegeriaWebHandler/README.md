...
- **Integration Connectors** — connectors on the Integration Daemon, with status filter pills and a manual **Refresh** that triggers a connector refresh. Each connector row expands to show its **catalog targets** — the metadata elements registered via the `CatalogTarget` relationship (e.g. a `SoftwareServerPlatform` for `OMAGServerPlatformCataloguer`, `GovernanceActionType` elements for `BabbageAnalyticalEngine`, `ReferenceCodeTable`/`TabularDataSet` elements for `JacquardDigitalProductLoom`). Catalog targets are fetched via `AutomatedCuration.get_catalog_targets` with `graphQueryDepth: 1` in the request body (limits traversal depth; prevents timeouts on connectors with large numbers of targets). (Note: pyegeria's sync `refresh_integration_connector` wrapper has a name bug; the handler calls the singular async method directly as a workaround.)
