...
> syntax surprises. **Phase D is also done** (2026-08-02): Topic and
> Perspective sub-collections (12 more: 4 Topic + 8 Perspective) were added
> additively — same Terms, no restructuring — via `topics_for()`/
> `perspectives_for()` inversions already in `overview_specs.py`, and
> verified live (spot-checked "App / AI Builder View" against
> `PERSP_KPIS["builder"]` and "Quality Metrics" against
> `TOPIC_KPIS["quality"]` — both matched exactly). Item 12
> ("`usage` notes for `illustrative`-tagged tiles") is vacuously complete:
> all 12 `_TILES` entries are `provenance: "live"` today — no
> `illustrative`-tagged tile exists in the registry yet (the dashboard's
> illustrative sections, e.g. Business Value, aren't `_TILES` entries at
> all — they're static HTML, a separate future scope). NEXT-24 is fully
> closed.
...
### Phase D — Illustrative tiles + follow-on sub-collections — ✅ done

12. ✅ `usage` notes for `illustrative`-tagged tiles — N/A, no such tile
    exists in `_TILES` (all 12 are `live`).
13. ✅ Topic/Perspective sub-collections (§2.2's "natural, cheap follow-ons") —
    additive `Add Member to Collection` links only, no restructuring. 12 new
    sub-collections (4 Topic + 8 Perspective), 59 new membership links,
    generated by `gen_dashboard_glossary.py` from `topics_for()`/
    `perspectives_for()`, validated and processed against a live server,
    verified browsable and correct in Egeria Explorer's Collections view.
