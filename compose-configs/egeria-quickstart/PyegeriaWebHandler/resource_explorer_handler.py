...
Advisor (see advisor_handler.py), same SSO handoff mechanism via
trellis_sso.py. No exclusive lock: Resource Explorer is multi-user with no
single-shared-instance constraint, so there is nothing to serialize access
to. Just a reachability check plus a token mint.

Auth
----
In DEMO_MODE the handoff endpoint requires a verified JWT user, mirroring
advisor_handler.py's _require_verified_or_local. Outside DEMO_MODE it
is open.
"""
...
# Same TTL-cached reachability pattern as advisor_handler.py's
