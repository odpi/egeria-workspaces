...
- PostgreSQL on `5442` (including database `dagster` with user `dagster_user`, with [pgvector](https://github.com/pgvector/pgvector) extension available)
- OpenLineage proxy on `6000`, `6001`
- Dagster Webserver (Dagit) on `8050`
...
docker compose -p egeria-shared-infra -f shared-infra.yaml build --pull proxy dagster-user-code
docker compose -p egeria-shared-infra -f shared-infra.yaml up -d --pull always proxy kafka postgres dagster-user-code dagster-webserver dagster-daemon
...
Rollback to a previous Kafka or Postgres image reference:

```bash
SHARED_KAFKA_IMAGE=<previous-kafka-image-ref> ./compose-configs/shared-infra/ensure-shared-infra.sh
SHARED_POSTGRES_IMAGE=<previous-postgres-image-ref> ./compose-configs/shared-infra/ensure-shared-infra.sh
```

### Dagster Integration & Database Setup

If you have an **already running** PostgreSQL container and do not want to destroy/recreate your database volume, run this command to initialize the Dagster database and database user manually:

```bash
docker exec -i egeria-shared-postgres psql -U postgres -d postgres -c \
...
Your pipelines are configured dynamically. You can edit the Python files in `runtime-volumes/dagster/user_code/repo.py` locally on your host. Once updated, navigate to `http://localhost:8050` and click the **Reload** button in the top-right corner to reload the code without restarting any Docker containers. Lineage events generated by your pipelines are automatically forwarded to Egeria's OpenLineage proxy.

For complete information on the Dagster integration design, service layouts, and code examples (including OpenLineage and Pyegeria), refer to [DAGSTER.md](file:///Users/dwolfson/localGit/antigravity-egeria-workspaces/egeria-workspaces/compose-configs/shared-infra/DAGSTER.md).
