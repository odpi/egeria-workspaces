...
from globals import (cocoMDS2Name, cocoMDS3Name, corePlatformURL, cocoCohort, devCohort, iotCohort, max_paging_size,
                     adminUserId, cocoMDS5Name, cocoMDS6Name)
...
def config_coco_core(url: str, userid: str):
    ...
    audit_config = {
        ...
        "databaseURL": "~{postgreSQLDatabaseURL}~?currentSchema=audit_logs", "databaseSchema": "audit_logs",
        "secretsStore": "~{secretsStore}~", "secretsCollectionName": "~{postgreSQLServerCollectionName}~"

        }

    p_client = Platform(cocoMDS2Name, platform_url, 'garygeeke')
    p_client.activate_platform(
        "coco_datalake", [cocoMDS2Name, cocoMDS3Name, cocoMDS5Name, cocoMDS6Name]
        )
    ...
    try:
        ...
        o_client.set_basic_server_properties("Governance Server", "Coco Pharmaceuticals", platform_url,
                                             mdr_server_user_id, mdr_server_password, max_paging_size)

        o_client.set_event_bus(event_bus_config)
        o_client.set_server_security_connection(security_connection_body)

        o_client.add_postgres_log_destinations(audit_config)

        pg_config_body = {
            "databaseURL": "~{postgreSQLDatabaseURL}~?currentSchema=mds2_gov_metadata_store",
            "databaseSchema": "mds2_gov_metadata_store", "secretsStore": "~{secretsStore}~",
            "secretsCollectionName": "~{postgreSQLServerCollectionName}~",
            }
        o_client.set_postgres_local_repository(pg_config_body)
    ...
    try:
        ...
        o_client.set_basic_server_properties("Research Server", "Coco Pharmaceuticals", platform_url,
                                             mdr_server_user_id, mdr_server_password, max_paging_size)
        ...
        pg_config_body = {
            "databaseURL": "~{postgreSQLDatabaseURL}~?currentSchema=mds3_research_metadata_store",
            "databaseSchema": "mds3_research_metadata_store", "secretsStore": "~{secretsStore}~",
            "secretsCollectionName": "~{postgreSQLServerCollectionName}~",
            }
        o_client.set_postgres_local_repository(pg_config_body)
        o_client.set_local_metadata_collection_id(metadataCollectionId)
    ...
    try:
        ...
        o_client.set_basic_server_properties("Business Systems", "Coco Pharmaceuticals", platform_url,
                                             mdr_server_user_id, mdr_server_password, max_paging_size)
        ...
        # o_client.set_xtdb_local_kv_repository()
        pg_config_body = {
            "databaseURL": "~{postgreSQLDatabaseURL}~?currentSchema=mds5_biz_sys_metadata_store",
            "databaseSchema": "mds5_biz_sys_metadata_store", "secretsStore": "~{secretsStore}~",
            "secretsCollectionName": "~{postgreSQLServerCollectionName}~",
            }
        o_client.set_postgres_local_repository(pg_config_body)
    ...
    try:
        ...
        o_client.set_basic_server_properties("Manufacturing", "Coco Pharmaceuticals", platform_url, mdr_server_user_id,
                                             mdr_server_password, max_paging_size)
        ...
        pg_config_body = {
            "databaseURL": "~{postgreSQLDatabaseURL}~?currentSchema=mds6_manufacturing_metadata_store",
            "databaseSchema": "mds6_manufacturing_metadata_store", "secretsStore": "~{secretsStore}~",
            "secretsCollectionName": "~{postgreSQLServerCollectionName}~",
            }
        o_client.set_postgres_local_repository(pg_config_body)
        ...
        access_service_options = {
            "SupportedZones": ["manufacturing"], "DefaultZones": ["manufacturing"]
            }
